...
const Navbar = () => {
    ...
    return (
        <nav className="flex items-center justify-center bg-white pb-6 z-20">
            <div className="flex w-[96%] items-center justify-between pt-5">
                {/* logo */}
                <Link to="/">
                                ...
                                <li key={index}>
                                    {
                                        link.title === "Services" ? (
                                            <div className="relative flex items-center gap-2 group">
                                                ...
                                                z-20 top-[10%] flex flex-col rounded-md p-6 lg:w-[265px] opacity-0
                                                        ...
                                                        subLinks.length ? (
                                                            subLinks.map( (subLink, index) => (
                                                                <Link to={`${subLink.link}`} key={index}>
                                                                    {/* <p className={`${matchRoute(subLink?.link) ? "text-[#5956E9]" : "text-richblack-600 relative"} pl-6 hover:text-[#5956E9] transition-all duration-700 hover:relative group-hover:transition-all group-hover:duration-700 group-hover:-translate-x-7`}>
                                                                        {subLink.title}
                                                                        <div className={`${matchRoute(subLink?.link) ? "w-[265px] h-[1px] bg-[#5956E9] absolute flex top-9 left-0" : "text-richblack-600"}`}></div>
                                                                    </p> */}
                                                                    <div className="relative hover:z-0">
                                                                        <p className="text-richblack-600 pl-6 transition-all duration-700 group-hover:transition-all group-hover:duration-700 group-hover:-translate-x-7 hover:text-[#5956E9] hover:z-10">
                                                                            {subLink.title}
                                                                        </p>
                                                                        <div className="w-[0px] h-[1px] bg-[#5956E9] absolute top-[50%] -left-[12px] hover:w-[240px] hover:h[1px] hover:bg-[#5956E9] hover:z-30"></div>
                                                                    </div>
                                                                </Link>
                                                            ))
                                                        ) : (<div></div>)
